...
export default function AddTimeToDateTool({ locale = "en" }: { locale?: ToolLocale }) {
  ...
  const result = useMemo(() => {
    ...
    const addedMs = daysNum * 86400000 + hoursNum * 3600000 + minutesNum * 60000;
    const resultDate = new Date(date.getTime() + addedMs);
    return {
  ...
  return (
    <section className="terminal-panel space-y-6">
      <div className="flex items-center justify-between gap-4 text-xs font-mono text-[var(--terminal-muted)]">
        <span className="terminal-accent">~/tools/add-time-to-date</span>
        <span>{text.command}</span>
      </div>

      <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="space-y-3 rounded-lg border border-[var(--terminal-border)] bg-[var(--terminal-panel-bg)]/40 p-4">
          <label htmlFor="add-time-date" className="block text-sm font-medium">
            {text.baseDate}
          </label>
          <input
            id="add-time-date"
      ...
      {"error" in result ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="rounded border border-[var(--terminal-border)] p-3">
            <div className="text-xs font-mono text-[var(--terminal-muted)]">timestamp ms</div>
